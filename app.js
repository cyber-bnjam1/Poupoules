...
function renderChickensList() {
    ...
    list.forEach(c => {
        ...
        div.innerHTML = `
            <button class="fav-btn ${c.isFavorite ? 'active' : ''}" onclick="toggleFavorite('${c.id}')"><i class="${heartClass}"></i></button>
...
window.archiveChicken = () => {
    ...
    if (idx > -1) {
        if(localChickens[idx].status === 'archived') {
            if(confirm("Supprimer définitivement ?")) localChickens.splice(idx, 1);
        } else {
...
function renderDashboard() {
    const now = new Date();
    const thisMonth = localEggs.filter(e => { const d = new Date(e.date); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); });
...
window.openAddEggModal = () => {
    ...
    actives.forEach(c => {
        ...
        div.onclick = () => { localEggs.push({ id: 'e'+Date.now(), chickenId: c.id, chickenName: c.name, date: new Date().toISOString() }); saveData(); renderDashboard(); modal.style.display = 'none'; };
        grid.appendChild(div);
    });
    modal.style.display = 'flex';
};
window.deleteEgg = (id) => { if(confirm("Supprimer ?")) { localEggs = localEggs.filter(e => e.id !== id); saveData(); renderDashboard(); }};
...
document.getElementById('form-transaction').addEventListener('submit', (e) => {
    ...
    const data = { amount: parseFloat(document.getElementById('trans-amount').value), date: document.getElementById('trans-date').value, type: document.getElementById('trans-type').value, category: document.getElementById('trans-category').value };
    if(id) { const idx = localTransactions.findIndex(t => t.id === id); if(idx>-1) localTransactions[idx] = { id, ...data }; }
    else { localTransactions.push({ id: 't'+Date.now(), ...data }); }
...
function renderMaintenance() {
    ...
    localTasks.sort((a,b) => getDaysDiff(b.lastDone) - getDaysDiff(a.lastDone)).forEach(t => {
        ...
        const li = document.createElement('li');
        li.innerHTML = `<div onclick="openEditTaskModal('${t.id}')"><strong>${t.title}</strong><br><small>Tous les ${t.frequency}j • Fait il y a ${diff}j</small></div><button class="btn-primary" style="padding:5px 10px; background:${diff>=t.frequency?'var(--danger)':'var(--success)'}" onclick="completeTask('${t.id}')">Fait</button>`;
        list.appendChild(li);
    });
}
...
window.openEditTaskModal = (id = null) => {
    const modal = document.getElementById('modal-edit-task'); document.getElementById('form-task').reset();
    if(id) { const t = localTasks.find(x => x.id === id); document.getElementById('task-id').value = t.id; document.getElementById('task-title').value = t.title; document.getElementById('task-freq').value = t.frequency; document.getElementById('btn-delete-task').style.display = 'block'; }
    else { document.getElementById('task-id').value = ""; document.getElementById('btn-delete-task').style.display = 'none'; }
    modal.style.display = 'flex';
};
document.getElementById('form-task').addEventListener('submit', (e) => {
    e.preventDefault(); const id = document.getElementById('task-id').value; const data = { title: document.getElementById('task-title').value, frequency: parseInt(document.getElementById('task-freq').value) };
    if(id) { const idx = localTasks.findIndex(t => t.id === id); if(idx>-1) localTasks[idx] = { ...localTasks[idx], ...data }; }
    else { localTasks.push({ id: 'task'+Date.now(), ...data, lastDone: new Date().toISOString() }); }
    saveData(); document.getElementById('modal-edit-task').style.display = 'none'; renderMaintenance();
});
