...
function sanitizeData(list, type) {
    ...
    if(type === 'chickens') {
        return list.map(c => ({
            ...
            health: c.health || 'healthy', // Nouveau champ
...
function saveData() {
    ...
    if(isDemoMode) {
        ...
        db.collection('users').doc(currentUser.uid).set(data).catch(alert);
    }
}
...
function loadLocalData() {
    ...
    if(raw) {
        try {
            ...
            localTasks = d.tasks || [];
        } catch(e) { console.error("Erreur lecture data", e); }
    } else {
        // Chargement des données démo si vide
        localChickens = DEMO_DATA.chickens;
        localEggs = DEMO_DATA.eggs;
        localTransactions = DEMO_DATA.transactions;
        localTasks = DEMO_DATA.tasks;
    }
}
