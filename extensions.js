// ====================================================================
// EXTENSIONS.JS - VERSION COMPLETE (TOUT INCLUS)
// Contient : Bio-Recycleur (Pain), Santé, Stock, Frigo, Ventes, etc.
// ====================================================================
...
// ==========================================
// 1. GESTION DES DONNÉES LOCALES (Get/Set)
// ==========================================
// Ces fonctions permettent de lire/écrire facilement et de déclencher la synchro Cloud
...
function setLocalData(key, value) {
    ...
    // DECLENCHE LA SYNCHRO CLOUD (via app.js)
    if (typeof window.saveData === 'function') window.saveData();
}

// Variables globales pour le runtime (synchronisées au démarrage par app.js)
window.recyclingHistory = getLocalData('poupoules_recycling_history', []);

// ==========================================
// 2. INJECTION DES CONTENEURS (HTML)
// ==========================================
window.renderExtensions = function() {
    ...
    // --- LANCEMENT DU RENDU DE TOUS LES WIDGETS ---
    renderAllWidgets();
};
...
// ==========================================
// 3. FONCTIONS BIO-RECYCLEUR (PAIN + COMPOST)
// ==========================================

window.renderRecyclerWidget = function() {
    const container = document.getElementById('recycler-widget-container'); 
    if (!container) return;
    
    // On recharge la variable depuis le stockage pour être à jour
    window.recyclingHistory = getLocalData('poupoules_recycling_history', []);
    ...
    let monthTotal = 0; 
    window.recyclingHistory.forEach(item => { 
    ...
    <div class="glass-card" style="background:linear-gradient(to right, rgba(255,255,255,0.8), rgba(200, 255, 200, 0.4)); border:1px solid rgba(52, 199, 89, 0.2);">
        ...
        <div style="margin-top:15px; font-size:10px; font-weight:bold; color:var(--success); text-transform:uppercase;">♻️ Compost (Boîte)</div>
        ...
        <div style="margin-top:15px; font-size:10px; font-weight:bold; color:#d35400; text-transform:uppercase;">🥖 Pain Dur</div>
...
window.addRecycling = function(e, qty) { 
    if(e) { e.preventDefault(); e.stopPropagation(); } 
    window.recyclingHistory.push({ date: new Date().toISOString(), qty: qty }); 
    setLocalData('poupoules_recycling_history', window.recyclingHistory); // Sauvegarde + Sync
...
window.editRecyclingTotal = function() { 
    const currentMonth = new Date().getMonth(); const currentYear = new Date().getFullYear(); 
    let currentTotal = 0; 
    window.recyclingHistory.forEach(item => { const d = new Date(item.date); if (d.getMonth() === currentMonth && d.getFullYear() === currentYear) currentTotal += item.qty; }); 
    ...
    if (newVal !== null && !isNaN(parseFloat(newVal))) { 
        ...
        window.recyclingHistory = window.recyclingHistory.filter(item => { const d = new Date(item.date); return !(d.getMonth() === currentMonth && d.getFullYear() === currentYear); }); 
        if (target > 0) { window.recyclingHistory.push({ date: new Date().toISOString(), qty: target }); } 
        setLocalData('poupoules_recycling_history', window.recyclingHistory); // Sauvegarde + Sync
        window.renderRecyclerWidget(); checkAchievements(); 
    } 
};

// ==========================================
// 4. AUTRES WIDGETS (SANTÉ, STOCK, FRIGO...)
// ==========================================
...
function renderSuppliesWidget() {
    ...
    const suppliesState = getLocalData('poupoules_supplies', {});
...
window.toggleSupply = (id) => { 
    const supplies = getLocalData('poupoules_supplies', {});
    if (supplies[id] === false) supplies[id] = true; else supplies[id] = false; 
    setLocalData('poupoules_supplies', supplies); // Sync
    renderSuppliesWidget(); 
};

function renderStockWidget() { 
    const container = document.getElementById('stock-widget-container'); if (!container) return; 
    const stockData = getLocalData('poupoules_stock', { quantity: 0, date: null });
...
window.saveStock = (e) => { 
    ...
    if (qty > 0) { 
        setLocalData('poupoules_stock', { quantity: qty, date: new Date().toISOString() }); // Sync
        document.getElementById('modal-stock').style.display = 'none'; 
        renderStockWidget(); 
    }
};
window.openStockModal = () => { document.getElementById('modal-stock').style.display = 'flex'; document.getElementById('stock-qty').value = getLocalData('poupoules_stock', {}).quantity || ''; };

// --- FRIGO ---
function renderFridgeWidget() { 
    const container = document.getElementById('fridge-widget-container'); if (!container) return; 
    const fridgeStock = getLocalData('poupoules_fridge_qty', 0);
...
window.updateFridge = (amount) => { 
    let fridge = getLocalData('poupoules_fridge_qty', 0);
    fridge += amount; 
    if (fridge < 0) fridge = 0; 
    setLocalData('poupoules_fridge_qty', fridge); // Sync
    renderFridgeWidget(); checkAchievements(); 
};
window.openSellModal = () => { 
    const fridge = getLocalData('poupoules_fridge_qty', 0);
    ...
    if (qty && qty > 0 && qty <= fridge) { 
        ...
        if (price) { 
            ...
            if (typeof localTransactions !== 'undefined') { 
                ...
                alert(`Vente de ${price}€ enregistrée !`); checkAchievements(); 
            } 
        } 
...
function renderJournalWidget() { 
    ...
    const notes = getLocalData('poupoules_notes', []);
    let notesHtml = ''; 
...
window.addNote = (e) => { 
    ...
    if (input.value) { 
        let notes = getLocalData('poupoules_notes', []);
        notes.push({ id: 'n'+Date.now(), text: input.value, date: new Date().toISOString() }); 
        setLocalData('poupoules_notes', notes); // Sync
        renderJournalWidget(); 
    }
};
...
function renderHallOfFame() { 
    ...
    const records = getLocalData('poupoules_records', { heaviest: 0, lightest: 1000 });
...
window.updateRecord = (type) => { 
    ...
    if(val && !isNaN(val)) { 
        ...
        let records = getLocalData('poupoules_records', { heaviest: 0, lightest: 1000 });
        if(type === 'heaviest') { 
            if(weight > records.heaviest || records.heaviest === 0) { records.heaviest = weight; alert("💪 Nouveau record !"); } else { alert("Pas un record !"); return; } 
        } else { 
            if(weight < records.lightest) { records.lightest = weight; alert("✨ Nouveau record !"); } else { alert("Pas un record !"); return; } 
        } 
        setLocalData('poupoules_records', records); // Sync
...
function renderHealthWidget() { 
    ...
    const health = getLocalData('poupoules_health', []);
...
window.saveHealthTreatment = (e) => { 
    e.preventDefault(); 
    const newItem = { id: 'h' + Date.now(), type: document.getElementById('health-type').value, product: document.getElementById('health-product').value, date: document.getElementById('health-date').value, nextDate: document.getElementById('health-next-date').value }; 
    let health = getLocalData('poupoules_health', []);
    health.push(newItem); 
    setLocalData('poupoules_health', health); // Sync
    document.getElementById('modal-health-treatment').style.display = 'none'; 
    renderHealthWidget(); checkAchievements(); 
};
...
function renderSalesWidget() { 
    ...
    const sales = getLocalData('poupoules_sales', []);
...
window.saveClientSale = (e) => { 
    e.preventDefault(); const price = parseFloat(document.getElementById('client-price').value); 
    const sale = { id: 's' + Date.now(), client: document.getElementById('client-name').value, qty: parseInt(document.getElementById('client-qty').value), price: price, status: document.getElementById('client-status').value, date: document.getElementById('client-date').value }; 
    let sales = getLocalData('poupoules_sales', []);
    sales.push(sale); 
    setLocalData('poupoules_sales', sales); // Sync
    
    if (sale.status === 'paid' && typeof localTransactions !== 'undefined') { localTransactions.push({ id: 't' + Date.now(), category: 'income', type: 'vente_oeufs', amount: price, date: sale.date }); if(window.saveData) window.saveData(); } 
    updateFridge(-sale.qty); 
    document.getElementById('modal-client-sale').style.display = 'none'; 
    renderSalesWidget(); checkAchievements(); 
};
window.markAsPaid = (id) => { 
    let sales = getLocalData('poupoules_sales', []);
    const idx = sales.findIndex(s => s.id === id); 
    if(idx > -1) { 
        if(confirm("Confirmer que " + sales[idx].client + " a payé ?")) { 
            sales[idx].status = 'paid'; 
            setLocalData('poupoules_sales', sales); // Sync
            
            if (typeof localTransactions !== 'undefined') { localTransactions.push({ id: 't' + Date.now(), category: 'income', type: 'vente_oeufs', amount: sales[idx].price, date: new Date().toISOString() }); if(window.saveData) window.saveData(); if(window.renderFinance) window.renderFinance(); } 
            renderSalesWidget(); checkAchievements(); 
        } 
    } 
};

// --- HELPERS (METEO, FINANCE, SUCCES...) ---
...
function checkAchievements() {
    ...
    const recyc = getLocalData('poupoules_recycling_history', []); let recycledTotal = 0; recyc.forEach(r => recycledTotal += r.qty);
    const sales = getLocalData('poupoules_sales', []); const health = getLocalData('poupoules_health', []); const fridge = getLocalData('poupoules_fridge_qty', 0); const records = getLocalData('poupoules_records', { heaviest:0 });
    const badges = [
