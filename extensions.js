// extensions.js - VERSION AFFICHAGE FORCE + PAIN
// Cette version force l'affichage du Bio-Recycleur en bas de page si elle ne trouve pas sa place.
...
// ==========================================
// INJECTION HTML (LOGIQUE DE FORCE)
// ==========================================
function injectExtensionContainers() {
    const dashboard = document.getElementById('view-dashboard');
    if (!dashboard) return; // Si pas de dashboard, on arrête

    // --- 1. WIDGETS HAUT (Météo / Almanach) ---
    if (!document.getElementById('weather-tip-container')) {
        ...
        if (title) title.insertAdjacentElement('afterend', div); else dashboard.prepend(div);
    }
    if (!document.getElementById('almanac-container')) {
        const div = document.createElement('div'); div.id = 'almanac-container'; div.style.marginBottom = "20px";
        const w = document.getElementById('weather-tip-container'); if(w) w.insertAdjacentElement('afterend', div);
    }

    // --- 2. STOCK (POINT DE REPÈRE) ---
    ...
    if (!stockContainer) {
        ...
        // On le met après l'almanach, ou sinon au début
        const almanac = document.getElementById('almanac-container');
        if (almanac) almanac.insertAdjacentElement('afterend', stockContainer);
        else dashboard.appendChild(stockContainer);
    }

    // --- 3. AUTRES WIDGETS AUTOUR DU STOCK ---
    ...
    if (!document.getElementById('supplies-widget-container')) {
        const div = document.createElement('div'); div.id = 'supplies-widget-container'; div.style.marginTop = "15px"; 
        // On l'insère APRES le stock
        if(stockContainer.nextSibling) stockContainer.parentNode.insertBefore(div, stockContainer.nextSibling);
        else stockContainer.parentNode.appendChild(div);
    }
    if (!document.getElementById('fridge-widget-container')) {
        const div = document.createElement('div'); div.id = 'fridge-widget-container'; div.style.marginBottom = "15px";
        stockContainer.parentNode.insertBefore(div, stockContainer);
    }

    // --- 4. BIO-RECYCLEUR (FORCE AFFICHAGE) ---
    if (!document.getElementById('recycler-widget-container')) {
        ...
        // TENTATIVE 1 : Après le matériel
        const supplies = document.getElementById('supplies-widget-container');
        if (supplies) {
            supplies.insertAdjacentElement('afterend', div);
        } 
        // TENTATIVE 2 : Après le stock
        else if (stockContainer && stockContainer.parentNode) {
            stockContainer.parentNode.insertBefore(div, stockContainer.nextSibling);
        } 
        // TENTATIVE 3 (ULTIME) : Tout en bas du dashboard
        else {
            dashboard.appendChild(div);
        }
    }

    // --- 5. AUTRES VUES (Finance, etc) ---
    injectOtherViewsContainers();
}

function injectOtherViewsContainers() {
    ...
    if (maintenance) {
        if (!document.getElementById('health-widget-container')) {
            const div = document.createElement('div'); div.id = 'health-widget-container'; div.style.marginTop = "50px"; maintenance.appendChild(div);
        }
        if (!document.getElementById('vet-widget-container')) {
            const div = document.createElement('div'); div.id = 'vet-widget-container'; div.style.marginTop = "30px"; maintenance.appendChild(div);
        }
    }
    if (dashboard && !document.getElementById('journal-widget-container')) {
    ...
    if (finance) {
        if (!document.getElementById('cost-price-container')) { const div = document.createElement('div'); div.id = 'cost-price-container'; const bal = finance.querySelector('.balance-card'); if(bal) bal.insertAdjacentElement('afterend', div); }
        if (!document.getElementById('savings-piggy-container')) { const div = document.createElement('div'); div.id = 'savings-piggy-container'; div.style.marginTop = "10px"; const cost = document.getElementById('cost-price-container'); if(cost) cost.insertAdjacentElement('afterend', div); }
        if (!document.getElementById('sales-register-container')) { const div = document.createElement('div'); div.id = 'sales-register-container'; div.style.marginTop = "20px"; div.style.marginBottom = "20px"; const tgt = document.getElementById('savings-piggy-container') || finance.querySelector('.balance-card'); if(tgt) tgt.insertAdjacentElement('afterend', div); }
    }
...
// ==========================================
// WIDGETS (LOGIQUE D'AFFICHAGE)
// ==========================================

// --- RECYCLEUR V3 ---
function renderRecyclerWidget() {
    // SECURITÉ : Si le conteneur n'existe pas, on force la création
    let container = document.getElementById('recycler-widget-container');
    if (!container) {
        injectExtensionContainers(); // On relance l'injection
        container = document.getElementById('recycler-widget-container');
        if (!container) return; // Si vraiment ça veut pas, on sort
    }
    ...
    container.innerHTML = `
        <div class="glass-card" style="background:linear-gradient(to right, rgba(255,255,255,0.8), rgba(200, 255, 200, 0.4)); border:1px solid rgba(52, 199, 89, 0.2);">
            ...
            <div style="margin-top:15px; margin-bottom:5px; font-size:10px; color:var(--success); font-weight:800; text-transform:uppercase; letter-spacing:0.5px;">♻️ Compost (Boîte IKEA)</div>
            ...
            <div style="margin-top:15px; margin-bottom:5px; font-size:10px; color:#d35400; font-weight:800; text-transform:uppercase; letter-spacing:0.5px;">🥖 Pain Dur</div>
