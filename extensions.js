// extensions.js - TOUTES FONCTIONS + MARGE SANTÉ CORRIGÉE
...
// MIGRATION & INIT
// ==========================================
// Cette fonction transfère tes vieilles données locales vers le nouveau système Cloud
window.migrateLocalStorageToCloud = function() {
    let hasChanged = false;
    // Liste des clés locales à vérifier
    const keys = {
    ...
    for (const [localKey, cloudKey] of Object.entries(keys)) {
        ...
        if (val) {
            try {
                // Cas spécial pour le frigo qui est un nombre simple
                if (cloudKey === 'fridge') window.localExtensionData[cloudKey] = parseInt(val);
                else window.localExtensionData[cloudKey] = JSON.parse(val);
                
                localStorage.removeItem(localKey); // On nettoie
                hasChanged = true;
            } catch(e) { console.error("Erreur migration " + localKey, e); }
        }
    }
    
    // Si on a migré des trucs, on force la sauvegarde Cloud immédiate
    if(hasChanged && window.saveData) window.saveData();
};

// ==========================================
// INJECTION HTML (MISE EN PAGE)
// ==========================================
function injectExtensionContainers() {
    ...
    // Finance (Coût + Tirelire + Ventes)
    if (finance) {
        if (!document.getElementById('cost-price-container')) {
            const div = document.createElement('div'); div.id = 'cost-price-container';
            const bal = finance.querySelector('.balance-card'); if(bal) bal.insertAdjacentElement('afterend', div);
        }
        if (!document.getElementById('savings-piggy-container')) {
            const div = document.createElement('div'); div.id = 'savings-piggy-container'; div.style.marginTop = "10px";
            const cost = document.getElementById('cost-price-container'); if(cost) cost.insertAdjacentElement('afterend', div);
        }
        if (!document.getElementById('sales-register-container')) {
            const div = document.createElement('div'); div.id = 'sales-register-container'; div.style.marginTop = "20px"; div.style.marginBottom = "20px";
            const tgt = document.getElementById('savings-piggy-container') || finance.querySelector('.balance-card');
            if(tgt) tgt.insertAdjacentElement('afterend', div);
        }
    }
...
window.addRecycling = (e, qty) => { if(e) { e.preventDefault(); e.stopPropagation(); } if(!window.localExtensionData.recycling) window.localExtensionData.recycling = []; window.localExtensionData.recycling.push({ date: new Date().toISOString(), qty: qty }); if(window.saveData) window.saveData(); renderRecyclerWidget(); checkAchievements(); if(e && e.target) { const btn = e.target; const originalText = btn.innerText; btn.innerText = "Ajouté !"; setTimeout(() => btn.innerText = originalText, 1000); } };
window.editRecyclingTotal = () => { if(!window.localExtensionData.recycling) window.localExtensionData.recycling = []; const currentMonth = new Date().getMonth(); const currentYear = new Date().getFullYear(); let currentTotal = 0; window.localExtensionData.recycling.forEach(item => { const d = new Date(item.date); if (d.getMonth() === currentMonth && d.getFullYear() === currentYear) currentTotal += item.qty; }); const newVal = prompt("Corriger le total de ce mois (kg) :", currentTotal.toFixed(2)); if (newVal !== null && !isNaN(parseFloat(newVal))) { const target = parseFloat(newVal); if (target < 0) return; window.localExtensionData.recycling = window.localExtensionData.recycling.filter(item => { const d = new Date(item.date); return !(d.getMonth() === currentMonth && d.getFullYear() === currentYear); }); if (target > 0) { window.localExtensionData.recycling.push({ date: new Date().toISOString(), qty: target }); } if(window.saveData) window.saveData(); renderRecyclerWidget(); checkAchievements(); } };
...
function checkAchievements() {
    ...
    const sales = window.localExtensionData.sales || []; const health = window.localExtensionData.health || []; const fridge = window.localExtensionData.fridge || 0; const records = window.localExtensionData.records || { heaviest:0 };
    const badges = [
...
window.renderExtensions = () => {
    // On n'appelle plus la migration ici car app.js le fait au chargement
