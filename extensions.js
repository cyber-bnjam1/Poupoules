// extensions.js - VERSION "FORCE BRUTE" + PAIN
// Le Bio-Recycleur est désormais "collé" au module Stock pour forcer son affichage.
...
// ==========================================
// INJECTION HTML (LA PARTIE CORRIGÉE)
// ==========================================
function injectExtensionContainers() {
    ...
    // 2. STOCK (Le point d'ancrage central)
    ...
    if (!stockContainer) {
        ...
        // On essaye de le placer intelligemment, sinon juste après l'almanach
    ...
    // 4. MATÉRIEL (Juste après le stock - Dedans ou après)
    if (!document.getElementById('supplies-widget-container')) {
        ...
        stockContainer.insertAdjacentElement('afterend', div);
    }
    
    // 5. FRIGO (Avant le stock)
    ...
    // 6. BIO-RECYCLEUR (CORRECTION : ANCRAGE FORCE SOUS LE MATERIEL)
    // On s'assure qu'il s'affiche en le collant après le bloc matériel (qui est lui-même après le stock)
    if (!document.getElementById('recycler-widget-container')) {
        const div = document.createElement('div'); 
        div.id = 'recycler-widget-container'; 
        div.style.marginTop = "25px"; // Espace propre
    ...
    if (maintenance) {
        ...
        if (!document.getElementById('vet-widget-container')) {
            const div = document.createElement('div'); div.id = 'vet-widget-container'; div.style.marginTop = "30px";
            maintenance.appendChild(div);
        }
    }
    if (!document.getElementById('journal-widget-container')) {
        ...
        if (act && act.previousElementSibling) act.previousElementSibling.insertAdjacentElement('beforebegin', div);
...
// HELPERS (Hall of Fame, Weather, Cost, Badges...)
